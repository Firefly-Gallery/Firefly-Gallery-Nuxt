<template>
    <div class="title-container relative">
        <div class="title" ref="titleRef">
            ...
            <span v-if="activeArtwork && activeArtwork.author" class="text-xs font-medium ml-2">背景：
  ...
  <div class="absolute bottom-0 left-0 right-0 flex p-2 justify-between items-center z-[-1]"
    ...
    <Icon name="heroicons:chevron-double-down" class="h-6 w-6 ud_animatiom" />
    <div class="badge badge-primary ml-2">v 1.3</div>
  </div>
</template>
...
const { enable_blur, parallax_bg } = useSettings()

const scrollY = useState('scrollY', () => 0)
watch(scrollY, (wScroll) => {
  if(!titleRef.value) return;
  let headerHeight = window.innerHeight;

  if(enable_blur.value) {
